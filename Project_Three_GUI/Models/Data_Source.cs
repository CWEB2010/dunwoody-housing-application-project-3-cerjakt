	...
	class Data_Source
	{
		...
		public ObservableCollection<Resident> readData()
		{
			...
			try
			{
				...
				while (!read.EndOfStream)
				{
					...
					residentList.Add(new Athlete(data[0], data[1], Convert.ToInt32(data[2]), Convert.ToInt32(data[3]), Convert.ToDouble(data[4]), Convert.ToInt32(data[5])));
				}
		...
		public void writeData(ObservableCollection<Resident> residentList)
		{
			...
			foreach (Resident x in residentList)
			{
				...
				write.WriteLine($"{x.name},{x.type},{x.id},{x.room_number},{x.rent_fee},{x.floor}");
			}
